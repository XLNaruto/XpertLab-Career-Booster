...
import { BookOpen, CheckCircle, Download, Trash2, Upload, X } from "lucide-react";
...
const EducationDetails = forwardRef<StepHandle, EducationDetailsProps>(({ onSaved }, ref) => {
  ...
  const handleDocumentChange = async (file: File, index: number) => {
    ...
    if (
      ...
      methods.setValue(`educations.${index}.url`, data?.url ?? data?.document ?? "");
    } else {
      toasterrormsg(response.data.message);
    }
  };
  ...
  useEffect(() => {
    const sub = methods.watch((_v, { name }) => {
      ...
      const result = educationDetailsSchema.safeParse({ educations: methods.getValues("educations") });
  ...
  return (
    <div className="space-y-5">
      {fields.map((field, idx) => {
        ...
        const isSscOrHsc = row?.educationType === "SSC" || row?.educationType === "HSC";
        ...
        return (
          <div key={field.id} className="bg-white/[0.4] border border-foreground/[0.06] rounded-2xl p-5">
            <div className="flex items-center justify-between mb-4">
              ...
              {idx > 0 && (
                <button type="button" onClick={() => handleRemoveField(idx)} className="text-xs text-primary font-semibold hover:opacity-70 flex items-center gap-1">
            ...
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <Select
                        ...
                        styles={selectStyles}
                        value={educationTypeList.find(o => o.value === field.value) || null}
                        onChange={(opt: any) => {
                  ...
                  {eduErrors?.educationType && (
                    <p className={errorClass}>{eduErrors.educationType.message as string}</p>
                  )}
                </div>
                {!isSscOrHsc && (
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <Controller
                        ...
                        control={control}
                        render={({ field }) => (
                          <input {...field} placeholder="Education" className={inputClass} />
                        )}
                      />
                    </div>
                    {eduErrors?.education && (
                      <p className={errorClass}>{eduErrors.education.message as string}</p>
                    )}
                  </div>
                )}
              </div>
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <Select
                        ...
                        styles={selectStyles}
                        value={boardList.find(o => o.value === field.value) || null}
                        onChange={(opt: any) => field.onChange(opt?.value || "")}
                      />
                    )}
                  />
                  {eduErrors?.boardId && (
                    <p className={errorClass}>{eduErrors.boardId.message as string}</p>
                  )}
                </div>
                <div>
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <Select
                        ...
                        styles={selectStyles}
                        value={instituteList.find(o => o.value === field.value) || null}
                        onChange={(opt: any) => field.onChange(opt?.value || "")}
                      />
                    )}
                  />
                  {eduErrors?.instituteId && (
                    <p className={errorClass}>{eduErrors.instituteId.message as string}</p>
                  )}
                </div>
              </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <Controller
                        ...
                        control={control}
                        render={({ field }) => (
                          <input {...field} placeholder="Percentage (%)" className={inputClass} />
                        )}
                      />
                    </div>
                    {eduErrors?.percentage && (
                      <p className={errorClass}>{eduErrors.percentage.message as string}</p>
                    )}
                  </div>
                </div>
              )}
              ...
              {isCompleted && (
                <div>
                        ...
                        <>
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <button
                              ...
                              onClick={(e) => {
                                e.stopPropagation();
                                methods.setValue(`educations.${idx}.document`, "");
                  ...
                  {eduErrors?.document && (
                    <p className={errorClass}>{eduErrors.document.message as string}</p>
                  )}
                </div>
              )}
            </div>
