...
const PersonalDetails = forwardRef<StepHandle, PersonalDetailsProps>(
  ({ onSaved }, ref) => {
    ...
    useEffect(() => {
      const sub = methods.watch((_v, { name }) => {
        ...
        const hasTraineeId = !!(values.traineeId && String(values.traineeId).trim() !== "");
        if (!hasTraineeId && (name === "password" || name === "confirmPassword")) {
          const pwd = (values.password || "").trim();
          const cpw = (values.confirmPassword || "").trim();
          if (name === "password" && !pwd) issueMessage = "Please Enter Password";
          else if (name === "confirmPassword" && !cpw) issueMessage = "Please Enter Confirm Password";
        ...
        if (issueMessage) {
          methods.setError(name as any, { type: "manual", message: issueMessage });
        } else {
    ...
    useImperativeHandle(
      ...
      () => ({
        save: async () => {
          ...
          const extraIssues: Array<{ field: keyof RegisterFormData; message: string }> = [];
          const hasTraineeId = !!(values.traineeId && String(values.traineeId).trim() !== "");
          const passwordValue = (values.password || "").trim();
          const confirmValue = (values.confirmPassword || "").trim();
          if (!hasTraineeId) {
            if (!passwordValue) {
              extraIssues.push({ field: "password", message: "Please Enter Password" });
            }
            if (!confirmValue) {
              extraIssues.push({ field: "confirmPassword", message: "Please Enter Confirm Password" });
            } else if (passwordValue && passwordValue !== confirmValue) {
              extraIssues.push({ field: "confirmPassword", message: "Password and Confirm Password didn't match" });
            }
          }

          if (!result.success || extraIssues.length > 0) {
            methods.clearErrors();
            if (!result.success) {
            ...
            extraIssues.forEach(({ field, message }) => {
          ...
          if (
            ...
            const data = response.data.data;
            if (data?.traineeId)
              methods.setValue("traineeId", String(data.traineeId));
      ...
      <div className="space-y-4">
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                {...register("lastName")}
                placeholder="Ex. Dhoni"
                className={inputClass}
              />
            </div>
            {errors.lastName && (
              <p className={errorClass}>{errors.lastName.message as string}</p>
            )}
          </div>
          <div>
            <label className={labelClass}>
          ...
          <div>
            ...
            />
            {errors.birthDate && (
              <p className={errorClass}>{errors.birthDate.message as string}</p>
            )}
          </div>
          <div>
            <label className={labelClass}>
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            />
            {errors.mobile1 && (
              <p className={errorClass}>{errors.mobile1.message as string}</p>
            )}
          </div>
          <div>
            ...
            />
            {errors.mobile2 && (
              <p className={errorClass}>{errors.mobile2.message as string}</p>
            )}
          </div>
        </div>
        ...
        <div className="border-t border-foreground/[0.08] pt-5 mt-5">
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div>
              <label className={labelClass}>
                Password {!traineeId && <span className="text-primary">*</span>}
              </label>
              <div className="relative">
                <div className={iconClass}>
            ...
            <div>
              <label className={labelClass}>
                Confirm Password {!traineeId && <span className="text-primary">*</span>}
              </label>
              <div className="relative">
                <div className={iconClass}>
