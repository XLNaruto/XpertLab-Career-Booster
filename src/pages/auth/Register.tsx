...
const Register = () => {
  ...
  const initialCompletedStep = clamp(
    parseInt(String(initialPayload.completedStep || "0"), 10) || 0,
    0,
    TOTAL_STEPS,
  );
  const initialStep = clamp(
    parseInt(String(initialPayload.step || "1"), 10) || 1,
    1,
    TOTAL_STEPS,
  );
            ...
            <div className="flex flex-col gap-2">
              {steps.map((s, i) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-all ${step === s.num ? "bg-gradient-to-br from-primary to-primary-light text-primary-foreground shadow-[var(--shadow-primary)]" : isCompleted ? "bg-green-500 text-white" : "bg-foreground/[0.08] text-muted-foreground"}`}
                    >
                      {isCompleted ? (
                        "✓"
                      ) : locked ? (
                    ...
                    <div>
                      <div
                        className={`text-[13px] font-semibold ${step === s.num || isCompleted ? "text-foreground" : "text-muted-foreground"}`}
                      >
                        {s.title}
                      </div>
          ...
          <div className="flex items-start justify-center p-10 overflow-y-auto max-h-screen">
            <div className="w-full max-w-[800px] bg-white/[0.55] border border-white/[0.85] rounded-3xl p-10 backdrop-blur-[28px] shadow-[var(--shadow-lg),inset_0_1px_0_white] animate-slide-up">
              <div className="text-center mb-8">
                ...
                <h2 className="font-serif text-[26px] font-bold text-foreground mb-1">
                  {steps[step - 1].title}
                </h2>
                ...
                <form onSubmit={methods.handleSubmit(onSubmit)}>
                  {step === 1 ? (
                    <PersonalDetails
                      ref={personalRef}
                      onSaved={() =>
                        handleSaved(methods.getValues("traineeId"))
                      }
                    />
                  ) : step === 2 ? (
                    <LocationDetails
                  ...
                  <div className="flex gap-3 mt-8">
                    <button
                      type="button"
                      onClick={() =>
                        step === 1 ? navigate("/login") : goToStep(step - 1)
                      }
          ...
          >
            ...
            >
              ...
              >
                ...
                />
                <CheckCircle2
                  className="w-11 h-11 text-white relative"
                  strokeWidth={2.4}
                />
              </motion.div>
              <motion.h2
